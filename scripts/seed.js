...
/**
 * Seed script: populates the Commodity, Index, and Trade collections from
 * the local comprehensive_dataset_wide.json file.
 *
 * Run with:  node scripts/seed.js
 *
 * Requires a reachable MongoDB/FerretDB instance.
 * Connection URI is read from MONGODB_URI env var or falls back to the
 * default local dev URI.
 */
...
const MONGO_URI =
  ...
  `mongodb://${process.env.MONGODB_APP_USER || "root"}:${process.env.MONGODB_APP_PASSWORD || "root"}@${process.env.MONGODB_HOST || "localhost"}:${process.env.MONGODB_GUEST_PORT || process.env.MONGODB_HOST_PORT || "27017"}/${process.env.MONGODB_INIT_DATABASE || "sci"}?authSource=admin`;
...
async function seedCommodities(rows) {
  ...
  for (const commodity of uniqueCommodities) {
    bulk.find({ commodity }).upsert().updateOne({
      $set: { commodity, category: Category(commodity) },
    });
  }
  const result = await bulk.execute();
  console.log(`  Commodities upserted: ${result.nUpserted + result.nModified}`);
}

// ─── Step 2: Seed Index ───────────────────────────────────────────────────────
async function seedIndex(rows) {
  console.log("\n[2/3] Seeding country index...");
  const seen = new Set();
  const entries = [];
  ...
  for (const row of rows) {
    ...
    let iso2 = null;
    try {
      iso2 = row.exporting_country_iso
        ? country2code.byIso(row.exporting_country_iso).iso2
        : null;
    } catch (_) {}
    ...
    entries.push({
      country,
      country_code: iso2,
      country_index: row.country_Index
        ? parseFloat((row.country_Index * 100).toFixed(2))
        : 0,
  ...
  const result = await bulk.execute();
  console.log(`  Index entries upserted: ${result.nUpserted + result.nModified}`);
}
...
async function seedTrades(rows) {
  ...
  // Build a commodity name→ObjectId lookup map up front
    ...
    for (const row of chunk) {
      ...
      for (let j = 0; j < commodityNames.length; j++) {
        ...
        commodity_tradeValue.push({
          ...
          commodity_index: parseFloat(commodityIndexes[j])
            ? parseFloat(parseFloat(commodityIndexes[j]).toFixed(2))
            : 0,
        });
      }
      commodity_tradeValue.sort((a, b) => b.export_value - a.export_value);

      const sumExport = exportValues.reduce((acc, v) => {
        const n = parseInt(v); return acc + (n || 0);
      }, 0);
      ...
      const doc = {
        ...
        Opportunity_Index: row.opportunity_Index
          ? parseFloat((row.opportunity_Index * 100).toFixed(2))
          : 0,
        Country_Index: row.country_Index
          ? parseFloat((row.country_Index * 100).toFixed(2))
          : 0,
        ...
        // parallel arrays kept for byPopi endpoint
      ...
      bulk
        .find({ Exporting_country: exportCountry, Importing_country: importCountry })
        .upsert()
        .updateOne({ $set: doc });
    }
...
(async () => {
  try {
    ...
    console.log("\nLoading dataset (this may take a moment for 459MB)...");
